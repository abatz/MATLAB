function varargout=notBoxPlot2(y,x,jitter,style,symbols,fc)
...
for k=1:length(X)
    ...
    if strcmp(style,'patch') 
      h(k).sdPtch=patchMaker(SD(k),[.7 .7 .7]);
    end
    
    if strcmp(style,'patch') || strcmp(style,'sdline')
      h(k).semPtch=patchMaker(SEM(k),[.5 .5 .5]);
      h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],'-k',...
           'linewidth',2);
    end
    ...
    for sym=1:17; 
        h(k).data=plot(thisX(sym)+J(sym), thisY(sym), symbols{sym},'color',fc{sym}); hold on; 
    end
end
...
if strcmp(style,'line')
    for k=1:length(X)     
        %Plot mean and SEM
        h(k).mu=plot(X(k),mu(k),'o','color',[.7 .7 .7],...
            'markerfacecolor',[.7 .7 .7],...
            'markersize',10);
        
        h(k).sem=plot([X(k),X(k)],[mu(k)-SEM(k),mu(k)+SEM(k)],'-k',...
            'linewidth',2);   
